...
import { CredentialFile, KE1, KE2, KE3, OpaqueClient, OpaqueConfig, OpaqueID, OpaqueServer, RegistrationRecord, RegistrationRequest, RegistrationResponse } from './node_modules/@cloudflare/opaque-ts/lib/src/index.js';
import { KVStorage, expectNotError } from './node_modules/@cloudflare/opaque-ts/lib/src/common.js';
async function test_full_registration(input, output) {
    // Setup
    const { cfg, password, server_identity, client_identity, credential_identifier, database } = input;
    // Client
    ...
    const server = new OpaqueServer(cfg, input.oprf_seed, input.server_ake_keypair, server_identity);
    ...
    const success = database.store(credential_identifier, Uint8Array.from(credential_file.serialize()));
...
async function test_full_login(input, output) {
    ...
    const ke2 = await server.authInit(deser_ke1, credential_file.record, credential_file.credential_identifier, credential_file.client_identity, context);
    expectNotError(ke2);
...
describe.each([OpaqueID.OPAQUE_P256, OpaqueID.OPAQUE_P384, OpaqueID.OPAQUE_P521])('full', (opaqueID) => {
...
//# sourceMappingURL=opaque_full.test.js.map